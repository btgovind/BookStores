...
class DescriptionActivity : AppCompatActivity() {
  ...
  var bookId:String?="100"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        aboutbook=findViewById(R.id.aboutthebook)
        txtbookDes=findViewById(R.id.bookDesc)
            ...
            val jsonRequest= object : JsonObjectRequest(Request.Method.POST,url,jsonParams, Response.Listener {
                try {
                    ...
                    if(success){
                        ...
                        val checkFav=DBAsynsTask(applicationContext,bookEntity,1).execute()
                        val isFav=checkFav.get()

                        if(isFav){
                            addtofav.text="Remove From Favourites"
                            val favouriteColor=ContextCompat.getColor(applicationContext,R.color.favColor)
                            addtofav.setBackgroundColor(favouriteColor)
                        }else{
                            addtofav.text="Add To Favourite"
                            val nofavouriteColor=ContextCompat.getColor(applicationContext,R.color.purple_200)
                            addtofav.setBackgroundColor(nofavouriteColor)
                        }

                        addtofav.setOnClickListener {
                            if(!DBAsynsTask(applicationContext,bookEntity,1).execute().get()){
                                val async=DBAsynsTask(applicationContext,bookEntity,2).execute()
                                val result=async.get()

                                if(result) {
                                    Toast.makeText(this@DescriptionActivity,"Book Added To Favourite",Toast.LENGTH_SHORT).show()

                                    addtofav.text="Remove From Favourite"
                                    val favouriteColor=ContextCompat.getColor(applicationContext,R.color.favColor)
                                    addtofav.setBackgroundColor(favouriteColor)
                                 }else{
                                ...
                                val async=DBAsynsTask(applicationContext,bookEntity,3).execute()
                                val result=async.get()
    ...
    class DBAsynsTask(val context:Context, val bookEntity:BookEntity, val mode: Int) :AsyncTask<Void,Void,Boolean>(){
